...
function createStyle(opts) {
    const { name, styleModuleSample, } = opts;
...
function attachStyle(opts) {
    const { styleModulePath, useSemicolons, } = opts;
    ...
    if (importDeclaration) {
        ...
        if (location !== importSource)
            console.log(`Component already has 'styles' imported from '${location}'.`);
        return;
    }
    //@ts-ignore
...
export function updateStyle(opts) {
    const { doCreateStyleModule, name, styleModuleSource, styleModuleSample, } = opts;
    if (!doCreateStyleModule)
        return;
    if (!styleModuleSource)
        createStyle(opts);
    else
        opts.styleModuleOutSource = styleModuleSource;
    attachStyle(opts);
}
